...
void run_Local_Search() {
    ...
    int i, nodei, j, nodej, posi=0, posj=0;
    double before, after, improvement=1000000.0;

    for (i = 1; i <= best_sol->steps - 2; i++)
    {
            nodei = best_sol->tour[i];                
        if (nodei == DEPOT || is_charging_station(nodei) == true) continue;
        for (j = i + 1; j <= best_sol->steps - 1; j++)
        {
                nodej = best_sol->tour[j];
            if (nodej == DEPOT || i == j || is_charging_station(nodej) == true) continue;
            before = get_distance(best_sol->tour[i - 1], nodei) +
                    get_distance(nodei, nodej) +
                    get_distance(nodej, best_sol->tour[j + 1]);
            after = get_distance(best_sol->tour[i - 1], nodej) +
                    get_distance(nodej, nodei) +
                    get_distance(nodei, best_sol->tour[j + 1]);
            if (after < before && after < improvement )
            {
                posi = i;
                posj = j; 
                improvement = after;
    ...
    {
        int 
            node1 = best_sol->tour[posi],
            node2 = best_sol->tour[posj];
        
        best_sol->tour[posi] = node2;
        best_sol->tour[posj] = node1;
        best_sol->tour_length = fitness_evaluation(best_sol->tour, best_sol->steps);
        //print_solution(best_sol->tour, best_sol->steps);
    }// if ((posi + posj) > 0)
